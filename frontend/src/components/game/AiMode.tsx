...
export default class aiMode extends Phaser.Scene {
  ...
  startGame() {
    ...
    socketClient.emit("startBall", {
      ballX: this.sendBallx,
      ballY: this.sendBallY,
      roomName: this.playerData.roomName,
      id: socketClient.id,
    });
    socketClient.on("startBallBack", (data) => {
      if (
        data.roomName == this.playerData.roomName &&
  ...
  create() {
    ...
    socketClient.on("gameOver", (data) => {
      ...
      if (this.renderReplayButton == true) {
        replayButton = this.add.text(450, 300, "Back To Home!", {
          color: "#fff",
        });
  ...
  scorep1(flag: boolean): void {
    if (flag == true) {
      ...
      socketClient.emit("goalScored", {
        p1score: this.p1score_number,
        roomName: this.playerData.roomName,
        id: socketClient.id,
        wishPlayer: this.playerData.wishPlayer,
      });
      if (p1s === 5) {
        this.p1victory.visible = true;
        socketClient.emit("replayClient", {
          roomName: this.playerData.roomName,
          id: socketClient.id,
        });
      }
