...
function OtpCard() {
  ...
  const handleEnableClick = () => {
    if (otp.length === 6) {
      axios
        .patch(`${backendUrl}/auth/enable/Otp`, { otp })
        .then((response) => {
  ...
  const handleDisableClick = () => {
    try {
      axios.patch(`${backendUrl}/auth/disable/Otp`).then((res) => {
        setOtpDisable(false);
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    axios
      .patch(`${backendUrl}/auth/generate/Otp`)
      .then((res) => {
  ...
  return (
    <>
      <div className={`2xl:w-[500px] w-8/12 relative`}>
        <div
          className={`${styles.playCard}  overflow-hidden h-[522px] w-full  max-w-[900px] 2xl:w-[500px] bg-color-0 rounded-[40px] flex flex-col items-center justify-center gap-[22px]`}
        >
          ...
          <div
            className={`w-[200px] h-[200px] bg-color-26 rounded-[10px] relative object-cover flex items-center justify-center border-[0.5px] border-color-6 ${
              otpDisable ? "" : "mt-5"
            }`}
          >
            {isLoading ? (
              <Skeleton className="w-[200px] h-[200px]  rounded-[10px]  bg-color-23" />
            ) : (
              <Image
                src={qrcode}
                alt="Qr Code"
            ...
            <>
              ...
              <div className=" flex w-full items-center justify-center gap-[10px] md:gap-[36px] md:flex-row flex-col ">
                <div
                  ...
                  className=" select-none w-[141px] h-[35px] bg-color-6 rounded-[22px] flex items-center justify-center cursor-pointer"
                >
                  <p className="text-[14px] text-color-0 font-nico-moji">
                    Enable
                  </p>
